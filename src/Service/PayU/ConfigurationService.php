...
class ConfigurationService
{
    ...
    final public const CONFIG_BLIK_POS_ID = 'blikPosId';

    final public const CONFIG_MD5_KEY = 'md5Key';

    final public const CONFIG_BLIK_MD5_KEY = 'blikMd5Key';

    final public const CONFIG_CLIENT_ID = 'clientId';

    final public const CONFIG_BLIK_CLIENT_ID = 'blikClientId';

    final public const CONFIG_CLIENT_SECRET = 'clientSecret';

    final public const CONFIG_BLIK_CLIENT_SECRET = 'blikClientSecret';

    final public const CONFIG_SANDBOX = 'sandbox';

    final public const CONFIG_BLIK_SANDBOX = 'blikSandbox';

    final public const CONFIG_SANDBOX_POS_ID = 'sandboxPosId';

    final public const CONFIG_BLIK_SANDBOX_POS_ID = 'blikSandboxPosId';

    final public const CONFIG_SANDBOX__MD5_KEY = 'sandboxMd5Key';

    final public const CONFIG_BLIK_SANDBOX__MD5_KEY = 'blikSandboxMd5Key';

    final public const CONFIG_SANDBOX_CLIENT_ID = 'sandboxClientId';

    final public const CONFIG_BLIK_SANDBOX_CLIENT_ID = 'blikSandboxClientId';

    final public const CONFIG_SANDBOX_CLIENT_SECRET = 'sandboxClientSecret';

    final public const CONFIG_BLIK_SANDBOX_CLIENT_SECRET = 'blikSandboxClientSecret';
    ...
    public function initialize(
        ...
        if ($isBlik === false) {
            ...
            if ($sandbox === null) {
                $sandbox = $this->isBlikSandBox($salesChannel);
            }
            ...
            if ($sandbox) {
                $this->initializePayUStaticConfiguration(
                    true,
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_POS_ID, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX__MD5_KEY, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_CLIENT_ID, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_CLIENT_SECRET, $salesChannel)
                );
            } else {
                $this->initializePayUStaticConfiguration(
                    false,
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_POS_ID, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_MD5_KEY, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_CLIENT_ID, $salesChannel),
                    $this->configurationService
                        ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_CLIENT_SECRET, $salesChannel)
                );
            }
        }
    }
    ...
    public function isCompleteConfiguration(): bool
    {
        ...
        if ($this->configurationService->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX)) {
            if (
                $this->configurationService
                    ->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_POS_ID, $salesChannel) === ''
                || $this->configurationService
    ...
    public function isBlikSandBox(?string $salesChannelId = null): bool
    {
        return $this->configurationService->getBool(
            self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX,
            $salesChannelId ?? $this->request->getCurrentRequest()?->get('sw-sales-channel-id')
        );
    }
    ...
    public function checkSavedCredentials(?Request $request = null): bool
    {
        ...
        if ($this->checkCredentials($request) === false) {
            return false;
        }

        try {
            $this->initialize(isBlik: true);
        } catch (\Throwable) {
            return false;
        }
    ...
    {
        $request ??= Request::createFromGlobals();
        $checkSandbox = $request->get('checkSandbox');
        $isBlik = (bool) $request->get('isBlik');
        ...
        try {
            if ($isBlik === false) {
                ...
                if ($checkSandbox) {
                    $this->initializePayUStaticConfiguration(
                        true,
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_POS_ID),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX__MD5_KEY),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_CLIENT_ID),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_SANDBOX_CLIENT_SECRET)
                    );
                } else {
                    $this->initializePayUStaticConfiguration(
                        false,
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_POS_ID),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_MD5_KEY),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_CLIENT_ID),
                        $request->get(self::CONFIG_PLUGIN_PREFIX . self::CONFIG_BLIK_CLIENT_SECRET)
                    );
                }
            }
        } catch (\Exception) {
